...
function getStringFromDB(buffer, start, length)
{
	...
	function readThumbnailImage(dataView, tiffStart, firstIFDOffset, bigEnd){
		...
		var thumbTags = readTags(dataView, tiffStart, tiffStart + IFD1OffsetPointer, TiffTags, bigEnd)

		// EXIF 2.3 specification for JPEG format thumbnail

		// If the value of Compression(0x0103) Tag in IFD1 is '6', thumbnail image format is JPEG.
		// Most of Exif image uses JPEG format for thumbnail. In that case, you can get offset of thumbnail
		// by JPEGInterchangeFormat(0x0201) Tag in IFD1, size of thumbnail by JPEGInterchangeFormatLength(0x0202) Tag.
		...
		if (thumbTags['Compression']) {
			...
			switch (thumbTags['Compression']) {
				case 6:
					...
					if (thumbTags.JPEGInterchangeFormat && thumbTags.JPEGInterchangeFormatLength) {
					// extract the thumbnail
						var tOffset = tiffStart + thumbTags.JPEGInterchangeFormat;
						var tLength = thumbTags.JPEGInterchangeFormatLength;
...
function readEXIFData(file, start)
{
	...
	// extract thumbnail
	tags['thumbnail'] = readThumbnailImage(file, start, firstIFDOffset, bigEnd);
